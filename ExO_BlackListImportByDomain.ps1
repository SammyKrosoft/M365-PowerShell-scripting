...
#  BlackListbydomain.ps1 <inputfile.txt>
#
#
...
# 
#  Up to 100 domains can be imported into a single rule
#
...
$maxlistsize=100
$Date=get-date 
$log = ".\blacklistimport.log"
$Admin=Get-CurrentUser
$RuleName = "Blacklist-by-domain $date by: " + $Admin.name 
 
Write-host $RuleName
"Admin: " + $admin.name |out-file $log 
$Rulename|out-file $log -Append
Write-host " "
Write-host "About to import:  $rulename from File: $blackListFile " -ForeGroundColor Green
Write-host " "
...
if ($newSafeDomainList.count -gt 0)
    ...
    if ($newSafeDomainList.count -lt $maxlistsize) 
        ...
        $Error.clear();
        New-TransportRule $ruleName -SenderDomainIs $newSafeDomainList -Quarantine $true
